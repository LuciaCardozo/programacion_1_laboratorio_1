...
int prestamos_alta(Prestamos array[], int size, int* contadorID)                          //cambiar prestamos
{
    ...
    if(array!=NULL && size>0 && contadorID!=NULL)
    {
        ...
        {
            (*contadorID)++;
            array[posicion].idPrestamos=generadorId();                                                       //campo ID
...
//            utn_getTexto("getTexto\n: ","\nError",1,TEXT_SIZE,1,array[posicion].varLongString);                 //mensaje + cambiar campo varLongString
            printf("\n Posicion: %d\n ID: %d\n varInt: %d\n",
                   posicion, array[posicion].idPrestamos,array[posicion].varInt);
            retorno=0;
        }
    }
    return retorno;
    ...
    if(array!=NULL && size>=0)
    {
        for(i=0;i<size;i++)
        {
            if(array[i].isEmpty==1)
                ...
                printf("\n ID: %d\n varInt: %d\n",
                       array[i].idPrestamos,array[i].varInt);      //cambiar todos
        }
        retorno=0;
    }
    return retorno;
