...
void diaContratacionMin(Publicidad* listPub,
                        Pantalla* listPan,
                        int lenPan,
                        int lenPub,
                        int valor)
{
   ...
   for(i=0;i<lenPub;i++)
   {
        ...
        {
            if(listPub[i].dias<=valor)
            {
                posPan=pan_findPanById(listPan,lenPan,listPub[i].idPantalla);
                if(posPan>=0)
                {
                    ///id nombre precio dia precioTotal
                    total=listPan[posPan].precio * listPub[i].dias;
                    printf("%d %s %f %d %f",listPan[posPan].idPantalla,listPan[posPan].nombre,
                                            listPub[i].dias,total);
                }
            }
        }
   }
